...
async def create_team(request: Request, body: CreateTeamRequest) -> CreateTeamResponse:
...
async def list_teams(request: Request) -> ListTeamsResponse:
    async with SessionLocal.begin() as db:
        user_id = await get_user_id(request)
...
async def list_teams_for_org(request: Request) -> ListTeamsResponse:
    async with SessionLocal.begin() as db:
        user_id = await get_user_id(request)
        if user_id == PUBLIC_USER:
            raise AuthError(
...
async def list_org_members(request: Request) -> ListTeamMembersResponse:
    async with SessionLocal.begin() as db:
        user_id = await get_user_id(request)
        if user_id == PUBLIC_USER:
            raise AuthError(
