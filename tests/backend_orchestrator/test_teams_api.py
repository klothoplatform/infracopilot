...
class TestCreateTeam(aiounittest.AsyncTestCase):
    ...
    @patch("src.backend_orchestrator.teams_api.get_user_id")
    @mock.patch(
    ...
    async def test_create_team(
        ...
        mock_get_teams_manager.return_value = TeamsManager(MagicMock(), MagicMock())
        response = await create_team(req, body, MagicMock())
    ...
    async def test_list_teams(
        ...
        req = Request(scope={"type": "http", "headers": []})
        mock_get_teams_manager.return_value = TeamsManager(MagicMock(), MagicMock())
        response: ListTeamsResponse = await list_teams(req, MagicMock())
    ...
    async def test_list_teams_for_org(
        ...
        req = Request(scope={"type": "http", "headers": []})
        mock_get_teams_manager.return_value = TeamsManager(MagicMock(), MagicMock())
        response: ListTeamsResponse = await list_teams_for_org(req, MagicMock())
        expected_result = Team(id="team1")
        self.assertEqual(response.teams, [expected_result])
